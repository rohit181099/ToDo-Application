...
async function getByTitle(req, res) {
  const searchEntries = await todos.searchbyTitle(req.body.keyword);
  return res.send(searchEntries.map( _.curry(createToDo)(req) ));
}

async function getByStatus(req, res) {
  const searchEntries = await todos.searchbyStatus(req.body.keyword);
  return res.send(searchEntries.map( _.curry(createToDo)(req) ));
}

async function getByPriority(req, res) {
  const searchEntries = await todos.searchbyPriority(req.body.keyword);
  return res.send(searchEntries.map( _.curry(createToDo)(req) ));
}

async function getByDate(req, res) {
  const searchEntries = await todos.searchbyDate(req.body.keyword);
  return res.send(searchEntries.map( _.curry(createToDo)(req) ));
}
...
const toExport = {
    ...
    getByTitle: { method: getByTitle, errorMessage: "Could not fetch todo" },
    getByStatus: { method: getByStatus, errorMessage: "Could not fetch todo" },
    getByPriority: { method: getByPriority, errorMessage: "Could not fetch todo" },
    getByDate: { method: getByDate, errorMessage: "Could not fetch todo" },
